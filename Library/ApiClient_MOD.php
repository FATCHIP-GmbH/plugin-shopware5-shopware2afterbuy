...
class ApiClient
{
    ...
    protected function sendRequest($request, $shopInterface = false)
    {
        ...
        if ($this->logger instanceof Logger) {
			...
			Achtung! Aktuell muss der Punkt 1 (default Afterbuy-Connnector Plugin Logging) noch per Hand auskommentiert werden.
			Schön wäre später eine Abfrage, die dynamisch über ein zusätzliches Parameterfeld in der Plugin Konfiguration gesteuert wird ("Verbesserungsvorschlag")
			
			Dazu muss die Datei
			[Shopware-Root]/custom/plugins/viaebShopwareAfterbuy/Services/ReadData/Internal/ReadOrdersService.php
			editiert werden und folgende Zeile auskommentiert werden: (in v1.0.3 müsste das in Zeile 89 stehen)
			
			$this->logger->error('No data received', array('Orders', 'Read', 'Internal'));
			
			-->
			
			// Manuell deaktiviert um statdessen das Advanced Log zu benutzen. (Schaltet die "No data recived" Meldungen ab).
			// $this->logger->error('No data received', array('Orders', 'Read', 'Internal'));
				...
				if (($xmlResponse->CallStatus == "Success") OR ($xmlResponse->success == "1")) {
					...
					if(strpos($requestFiltered, $needle) !== false){			// wenn der Request String "&ArtikelStammID=" enthält, handelt es sich um die Übergabe einer Bestellung.
						if ($advLogLevel >= 3) {
							...
							$this->logger->error($statusText . $statusText2, $content);
						}
					} else{
						if ($advLogLevel >= 4) {
